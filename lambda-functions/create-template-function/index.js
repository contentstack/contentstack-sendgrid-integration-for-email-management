...
const { managementToken, baseUrl, sendGridBaseUrl, sendGridAPIKey, workflowId, userId, unSubscribeId } = process.env;
...
const updateWorkFlowStage = async (contentTypeUid, uid, id, stageUid) => {
...
const mainHandler = async (entryData) => {
  ...
  if (res.status === 201) {
    ...
    if (workflowResponse.status === 200) {
      ...
      await updateWorkFlowStage(entryData.data.content_type.uid, entryData.data.entry.uid, res.data.id, stageId.workflow_stages[1].uid);
